...
function isEmptySingUp(){
    if(inSingUpName.value!==""&&inSingUpMail.value!==""&&inSingUpPass.value!==""){
...
function isMailAndPass(mail , pass){
    for(let i=0; i<infosFeld.length;i++){
        if(infosFeld[i].singUpMail.toLowerCase()===mail.value&&infosFeld[i].singUpPass.toLowerCase()===pass.value.toLowerCase()){
           return true;
        }
